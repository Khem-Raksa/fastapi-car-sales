...
def get_car_by_id(request : Request,id:int = Path(...,ge=0,lt=1000)):
    #Get value by key of dictionary
    car = cars.get(id)
...
def add_cars(
    ...
    body_cars = [Car(make=make,model=model,year=year,price=price,engine=engine,autonomous=autonomous,sold=sold,min_id=min_id)]
...
def delete_car(request:Request, id : int = Query(...)):
    car = cars.get(id)
    if car:
        cars.pop(id)
    return RedirectResponse(url="/cars")
...
async def update_car(id:int, car:Car = Body(...)):
    stored = cars.get(id)
    if not stored:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,details="Could not find car with given id")
    stored = Car(**stored)
    new = car.dict(exclude_unset=True) #convert it to dictionary, exclude anything that wasn't changed
